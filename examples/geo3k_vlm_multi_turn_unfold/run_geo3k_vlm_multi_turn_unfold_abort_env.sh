...
# Multi-turn unfold VLM RL training on geo3k dataset (with env abort support)
# Same as run_geo3k_vlm_multi_turn_unfold.sh but uses rollout_abort_env.py which:
#   - Checks state.aborted cooperatively in generate() each turn
#   - Registers/unregisters envs for active cleanup during abort
#   - Custom generate_rollout entry that cleans up envs before SGLang abort
# Usage:
#   SLIME_SCRIPT_TRAIN_BACKEND=megatron ./run_geo3k_vlm_multi_turn_unfold_abort_env.sh
#   SLIME_SCRIPT_MODEL_NAME=Qwen3-VL-8B-Thinking ./run_geo3k_vlm_multi_turn_unfold_abort_env.sh
