...
# Multi-turn unfold VLM RL training on geo3k dataset (async mode)
...
# Async mode: actor and rollout run on separate GPUs (NUM_GPUS split evenly).
# NUM_GPUS must be even.
# Usage:
#   SLIME_SCRIPT_TRAIN_BACKEND=megatron ./run_geo3k_vlm_multi_turn_unfold_async.sh
#   SLIME_SCRIPT_MODEL_NAME=Qwen3-VL-8B-Thinking ./run_geo3k_vlm_multi_turn_unfold_async.sh
...
# Async: split NUM_GPUS evenly between actor and rollout (no --colocate)
MISC_ARGS=(
   --actor-num-nodes 1
